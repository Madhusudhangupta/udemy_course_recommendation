...
def recommend_course(df, title, cosine_mat, numrec):
    ...
    final_recommend_courses = rec_df[['course_title', 'Similarity_Score', 'url', 'price', 'num_subscribers']]
    return final_recommend_courses.head(numrec)


def searchitem(item, df):
    result_df = df[df['course_title'].str.contains(item)]
    top6 = result_df.sort_values(by='num_subscribers', ascending=False).head(6)
    return top6


def extractfeatures(rectdf):
    course_url = list(rectdf['url'])
    course_title = list(rectdf['course_title'])
    course_price = list(rectdf['price'])
...
def hello_world():
    if request.method == "POST":
        ...
        try:
            ...
            resultdf = searchitem(titlename, df)
